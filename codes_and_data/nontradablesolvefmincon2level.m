...
function [c, ceq, gc, gceq]=nontradablesolvefmincon2level(x)
c=[];
...
global kappaO kappaG kappaS FFelast Selast eqnormalisation
...
cmatrix9=((1-alpha-beta).*Y-lamFF.*(kappaS.^(1/Selast).*S.^(1-1/Selast)+(1-kappaS).^(1/Selast).*FF.^(1-1/Selast)).^(Selast/(Selast-1)))./OT;
%cmatrix10=(FF*eqnormalisation+lamFF-sqrt(lamFF.^2+(FF*eqnormalisation).^2)).^2;
cmatrix10=(S-kappaS./(1-kappaS).*(pFF./pe).^Selast.*FF)./ST;
%cmatrix11=pe-pFF+lamFF;
cmatrix11=lamFF-(kappaS.*pe.^(1-Selast)+(1-kappaS).*pFF.^(1-Selast)).^(1/(1-Selast));
